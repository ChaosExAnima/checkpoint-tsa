package gameGraphics
{
	...
	public class Floor extends Sprite
	{
			...
			for(i = 0; i < depth_array.length; i++) // For all in the depth array...
			{
				if(depth_array[i]!=null) // ... and is a person ...
				{
					if(!passArray[depth_array[i]].obsolete) // ... and is not obsolete
					{
						...
						pickupBox(passArray[depth_array[i]].getFootPrintX(),
								  passArray[depth_array[i]].getFootPrintY(),
								  passArray[depth_array[i]].getFootPrintW(),
								  passArray[depth_array[i]].getFootPrintH()); // Removes collision block
						//removeChild(passArray[depth_array[i]]); // Removes sprite
						moveOn(passArray[depth_array[i]]);
					}			
				}
			}
		...
		private function moveOn(pass:PassengerG):void {
			var line:LineG = pass.getLine();
			line.receivePass(pass);
		}
