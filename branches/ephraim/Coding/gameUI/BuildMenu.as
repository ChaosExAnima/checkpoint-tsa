	...
	public class BuildMenu extends MovieClip implements IMenu {
		...
		public function init():void {
			for each (var button:Sprite in _buttons) {
				...
				button.addEventListener(MouseEvent.MOUSE_OUT, clearDispData);
			}
			_UI.showStations();
			showAvailableUnits();
			clearDispData(null);
		...
		private function showAvailableUnits():void {
			...
			for each (var item:String in _gameXML.getLevel(TheGame.getLevel(), "items")) {
				var machine:Button;
				if (item != "NewLine") {
		...
		private function cancelHandler(e:MouseEvent):void {
			...
			if (_UI.gSecCheckUnit) {
				for each (var stat:StationG in _UI.stations) {
					for (var i:int = 0; i < stat.spotArray.length; i++) {
						var spot:Sprite = stat.spotArray[i];
						if (spot is GSpot) {
							spot.removeEventListener(MouseEvent.CLICK, placeMachine);
						}
						if (spot == _UI.gSecCheckUnit) {
							stat.removeSecurityCheckUnitG(i);
							_UI.gSecCheckUnit = null;
						}
					}
				}
				if (_UI.gSecCheckUnit.parent == Globals.airport) {
					Globals.airport.removeChild(_UI.gSecCheckUnit);
					_UI.gSecCheckUnit = null;
				} else if (_UI.gSecCheckUnit.parent == Globals.UI) {
					Globals.UI.removeChild(_UI.gSecCheckUnit);
					_UI.gSecCheckUnit = null;
				}
			}
		}
		...
		private function placeMachine(event:Event):void {
			...
			Globals.airport.enableMouseOnLines();
		...
		private function clearDispData(e:MouseEvent):void {
			this.unitName.text = "";
			this.unitPrice.text = "";
		...
		private function dispMachineData(e:MouseEvent):void {
			...
			var descrText:String = XMLmachineData.getXML(machName, "description");
			
			var results:Array = descrText.split("|");
			Globals.infoBox.addText(results[0]);
		}
	}
}
		/**/